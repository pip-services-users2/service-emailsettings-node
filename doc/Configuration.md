...
Email settings microservice configuration structure follows the 
...
- options: any - Misc options
  - magic_code: string - magic code to verify emails (default: disabled)
  - verify\_on\_create: string - automatically sends verification email on settings creation
  - verify\_on\_update: string - automatically sends verification email when user changes email
...
Example:
```yaml
- descriptor: "service-emailsettings:service:seneca:default:1.0"
  connection:
    protocol: "http"
    host: "0.0.0.0"
    port: 3000
